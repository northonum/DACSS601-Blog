...
player_links <- atp_base %>%
  filter(season==vSeason, loser_name%in%player_vert$player_name, winner_name%in%player_vert$player_name)%>%
  select(loser_name, winner_name)%>%
  group_by(loser_name, winner_name)%>%
  summarize(loser_name, winner_name, weight=n())%>%
  distinct()

player_vert<-player_vert%>%
  merge(player_links%>%
          group_by(winner_name)%>%
          summarize(winner_name, total_weight=sum(weight))%>%
          select(winner_name, total_weight)%>%
          distinct()%>%
          rename(player_name = winner_name), by=c("player_name"), all.x=TRUE)%>%
          replace_na(list(total_weight = 0))
...
net <- graph_from_data_frame(d=player_links, vertices=player_vert, directed=T)

#deg <- degree(net, mode="all")

V(net)$size <- (V(net)$total_weight+1)*3
...
# plot it
plot(net,main="1984 Network Diagram of Matches between Top 10 Players", sub="Edge weighted by number of matches\nVertex size = total wins against Top 10", edge.arrow.size=.2, edge.color="grey80",
     vertex.color="lightblue", vertex.frame.color="grey80",
     vertex.label=V(net)$player_name, vertex.label.color="black", vertex.label.font=1, vertex.label.cex=.8)
